...
import { mergeSchoolRows } from "../scripts/merge-reviewed-duplicate-schools-round15";

const scriptPath = "scripts/merge-reviewed-duplicate-schools-round15.ts";

test("round15 only merges the four reviewed duplicate rows into canonical schools", () => {
...
test("round15 is dry-run by default and applies under one short locked transaction", () => {
  const script = readFileSync(scriptPath, "utf8");
...
test("round15 preserves target values and records source rows, aliases, and conflicts", () => {
  const script = readFileSync(scriptPath, "utf8");

  assert.match(script, /school_data_merged_sources_round15/);
...
test("round14 merge behavior keeps canonical values and only fills gaps", () => {
  ...
  assert.equal((merged.attrs.school_data_merged_sources_round15 as Array<{ source_id: number }>)[0].source_id, 4968);
  assert.deepEqual(merged.conflicts.map((item) => item.field).sort(), ["school_nature", "tier"]);
});

test("round15 migrates every live school reference and handles unique-key collisions", () => {
  const script = readFileSync(scriptPath, "utf8");
...
test("round15 writes auditable dry-run and applied reports and is idempotent", () => {
  const script = readFileSync(scriptPath, "utf8");
