...
export async function getPipelineStats() {
  const rows = await query<{
    ...
    select
      ...
      (select count(*)::int from catalog.relations) relations,
      ...
      (select count(*)::int from catalog.relations where review_status='accepted') "acceptedRelations",
      (select count(*)::int from catalog.relations where review_status='provisional') "provisionalRelations",
      (select count(*)::int from catalog.relations where review_status='accepted' and published_at is null) "unreleasedRelations",
  ...
  const stages: PipelineStage[] = [
    ...
    },
    {
      ...
      status: "ok",
      hint: `${d.acceptedRelations} 已接受 · ${d.provisionalRelations} 官方直发（provisional）`,
    },
    {
      key: "publish",
      label: "发布",
