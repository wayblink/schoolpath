...
- Discovery must classify generated external-link templates by URL origin before enqueueing them. For example, `buildLianjiaAddrUrl(addr)` may produce a path-shaped string such as `/addr`; that value is not a House same-origin resource and must not become a fetched or unavailable source row.
