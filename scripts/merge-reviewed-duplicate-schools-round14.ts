...
export const REVIEWED_MERGES: ReviewedMerge[] = [
  {
    sourceId: 4782, targetId: 5107,
    sourceName: "迅行初级",
    sourceDistrict: "浦东",
    targetName: "上海市民办迅行中学",
    targetDistrict: "虹口",
    type: "middle",
    evidence: "错误区属的 XHS 简称行；虹口区 2025 年民办学校官方表唯一确认目标全称和玉田路 211 号地址。",
  },
  {
    sourceId: 5266, targetId: 5107,
    sourceName: "上海市民办迅行中学",
    sourceDistrict: "虹口",
    targetName: "上海市民办迅行中学",
    targetDistrict: "虹口",
    type: "middle",
    evidence: "同区同学段、同官方全称、同地址、同坐标且引用来源相同的精确重复行。",
  },
  {
    sourceId: 4968, targetId: 5552,
    sourceName: "新华初级",
    sourceDistrict: "黄浦",
    targetName: "上海市民办新华初级中学",
    targetDistrict: "虹口",
    type: "middle",
    evidence: "错误区属简称行的地址和坐标均指向虹口中州路 102 号；虹口区官方表唯一确认目标全称和民办性质。",
  },
  {
    sourceId: 4577, targetId: 5552,
    sourceName: "上海市新华初级中学",
    sourceDistrict: "虹口",
    targetName: "上海市民办新华初级中学",
    targetDistrict: "虹口",
    type: "middle",
    evidence: "同区同学段且地址、坐标完全一致；源行 POI 也指向民办新华，官方表确认规范全称含“民办”。",
  },
  {
    sourceId: 6054, targetId: 4642,
    sourceName: "市西初级中学",
    sourceDistrict: "静安",
    targetName: "上海市市西初级中学",
    targetDistrict: "静安",
    type: "middle",
    evidence: "同区同学段简称行；目标行已有静安区官方名称规范化记录，坐标相距不足 100 米。",
  },
];
...
export function mergeSchoolRows(
  ...
  const attrs = {
    ...targetAttrs,
    aliases,
    school_data_merged_sources_round14: [
      ...(Array.isArray(targetAttrs.school_data_merged_sources_round14)
        ? targetAttrs.school_data_merged_sources_round14
        : []),
      {
        source_id: source.id,
    ...
    school_data_merge_conflicts: [
      ...(Array.isArray(targetAttrs.school_data_merge_conflicts)
        ...
        : []),
      ...conflicts.map((conflict) => ({ ...conflict, detected_at: mergedAt, round: 14 })),
    ],
  };
...
async function main() {
  ...
  const reportDir = path.join(process.cwd(), ".tmp", "merge-reviewed-duplicate-schools-round14", stamp);
...
if (process.argv[1]?.endsWith("merge-reviewed-duplicate-schools-round14.ts")) {
  main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
